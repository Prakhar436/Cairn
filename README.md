...
A small, offline job-application tracker. One HTML file, no backend, no account, no build step — open it in a browser and your data stays on your device.

**Live site:** https://prakhar436.github.io/Cairn/
...
- **Automatic priority scoring** (Low → Medium → High → Critical) based on days since a listing was added, with active listings auto-sorted so the most urgent is always on top
- **A confirmation + timer reset** when a listing reaches "Follow up with HR," since that's the point the real-world clock restarts on a listing
